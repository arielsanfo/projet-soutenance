...
//----------------------------------------------------------------------------//
// SERVICE: Gestion des Fournisseurs (SupplierService)
//----------------------------------------------------------------------------//
class SupplierService {
  final Isar isar;

  SupplierService(this.isar);
  ...
  Future<Supplier?> getSupplierById(Id id) async {
...
class SupplierOrderService {
  ...
  Future<SupplierOrder> createSupplierOrder({
    ...
    await isar.writeTxn(() async {
      ...
      if (createdByUserId != null)
        order.createdByLink.value = await isar.users.get(createdByUserId);
  ...
  Future<void> receiveSupplierOrderItems({
    ...
    await isar.writeTxn(() async {
      ...
      if (orderItem.productLink.value == null ||
          orderItem.supplierOrderLink.value == null) return;

      orderItem.quantityReceived =
          (orderItem.quantityReceived ?? 0) + quantityReceived;
      ...
      await productService.updateStock(
        ...
        quantityChange:
            quantityReceived, // Positif car c'est une entrée de stock
      ...
      bool allReceived = order.supplierOrderItems
          .every((item) => item.quantityReceived! >= item.quantityOrdered!);

      order.status = allReceived
          ? SupplierOrderStatusIsar.received
          : SupplierOrderStatusIsar.partiallyReceived;
  ...
  Future<void> deleteSupplierOrder(Id orderId) async {
    await isar.writeTxn(() async {
      await isar.supplierOrders.delete(orderId);
    });
  }
}
